...
def movielookup(mykey: str, searchstring: str, movietype: str=None, yearofrelease: str=None):
    ...
    try:
        ...
        if movietype != None:
            api = f"{OMDBURL}apikey={mykey}&s={searchstring}&type={movietype}" 
        elif yearofrelease != None:
            api = f"{OMDBURL}apikey={mykey}&s={searchstring}&y={yearofrelease}"
        elif (movietype != None) and (yearofrelease != None):
            f"{OMDBURL}apikey={mykey}&s={searchstring}&type={movietype}&y={yearofrelease}"
        else:
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            3) Search for Movies Containing String, Type and Year of Release
            99) Exit""")
        ...
        if answer in ["1","2","3"]:
            ...
            if answer == "1":
                ...
                resp = movielookup(mykey, searchstring, movietype)  # maybe you can write this code!
                                     # restart the while loop
            elif answer == "3":
                movietype = input("MovieType: ")
                yearofrelease = input("YearofRelease: ")
                resp = movielookup(mykey, searchstring, movietype, yearofrelease)
                continue
